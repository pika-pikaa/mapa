...
class DatabaseManager:
    ...
    def import_slaskie_from_csv(self, csv_path: str = "data/baza-slaskie.csv") -> bool:
        """
        Importuje atrakcje Śląskiego z pliku CSV
        """
        with self._lock:
            conn = self._get_connection()
            try:
                ...
                # Sprawdź czy dane ze Śląskiego już istnieją
                cursor.execute("SELECT COUNT(*) FROM places WHERE location LIKE '%Kraków%' OR location LIKE '%Katowice%'")
                if cursor.fetchone()[0] > 10:
                    return True
                ...
                conn.commit()
                if imported > 0:
                    print(f"Zaimportowano {imported} atrakcji ze Śląskiego.")
                return True

            except Exception as e:
                print(f"Błąd podczas importu Śląskiego: {e}")
                conn.rollback()
                return False
            finally:
                conn.close()

    def import_lodzkie_from_csv(self, csv_path: str = "data/baza-lodzkie.csv") -> bool:
        """
        Importuje atrakcje Łódzkiego z pliku CSV
        """
        with self._lock:
            conn = self._get_connection()
            try:
                ...
                # Sprawdź czy dane z Łódzkiego już istnieją
                cursor.execute("SELECT COUNT(*) FROM places WHERE location LIKE '%Łódź%' OR location LIKE '%Piotrków%'")
                if cursor.fetchone()[0] > 10:
                    return True
                ...
                conn.commit()
                if imported > 0:
                    print(f"Zaimportowano {imported} atrakcji z Łódzkiego.")
                return True

            except Exception as e:
                print(f"Błąd podczas importu Łódzkiego: {e}")
                conn.rollback()
                return False
            finally:
                conn.close()
